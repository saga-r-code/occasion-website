...
router.post('/api/admin/signup', [
    ...
    try {
        ...
        const existingEmail = await pool.query(`SELECT * FROM admin_signup WHERE email = ?`, [userInput.email]);
        ...
        const query = `INSERT INTO admin_signup (username, email, password) VALUES (?, ?, ?)`;
        ...
        return res.status(201).send({ message: "Admin Account Created Successfully", token });
    } catch (error) {
